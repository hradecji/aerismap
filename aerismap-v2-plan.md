...
|---|---|---|---|
| D1 | Hosting budget | **$0/mo** — Cloudflare free plan + GitHub Actions for ingestion | Cloudflare only *serves*; all fetching/normalizing runs in an hourly GitHub Actions job (needed because a free-plan cron Worker gets 10 ms CPU and 50 subrequests — unusable) |
| D2 | Domain | **workers.dev for now** | No edge caching (Cache API is non-functional on workers.dev). Every API hit = one Worker invocation + one R2 read. Fine at hobby traffic; custom domain is a later drop-in upgrade |
...
├── worker/            Cloudflare Worker: /api/v1/* from R2 binding, else static assets
...
- **API routes** (Worker → R2 binding, ~5 routes):
...
- Serve gzip bodies with `Content-Encoding: gzip` passthrough; set `ETag` (from R2) + `Cache-Control: public, max-age=300` so browsers/proxies revalidate cheaply even without edge cache. RFC 9457 `problem+json` errors (kept from v1's contract).
...
- **R2 budget:** writes ~10 objects/hour = ~7.2k Class A ops/month (free tier: 1M). Reads = Class B, 10M/month free. Storage: `latest/` is <30 MB total. Nowhere near any cap.
...
1. Daily registry refresh: `GET /v3/locations?bbox=-25,34,45,72&monitor=true&limit=1000&page=N` → station id/coords/country registry (kept as an R2 object between runs).
...
- **Caveats**: 7-day file retention, no SLA — the previous hour's artifacts in R2 are the fallback (serve stale with a freshness banner). The official `.om` readers (`@openmeteo/file-reader` TS/WASM, and the Python reader) are **GPL-2.0-only** — see §7.
...
- Upload to R2 via the S3-compatible API (scoped R2 API token in repo secrets). Write to `latest/` atomically (write `new/` then copy, or versioned keys + meta pointer) so a mid-upload map load never mixes hours.
...
6. **Double-counting** across community networks — moot until openSenseMap is added; then dedup by coordinate proximity.

---

## 9. Milestones

- **M0 — Skeleton (a weekend):** monorepo scaffold; Next static export + MapLibre + OpenFreeMap renders; Worker serves assets + a stub `/api/v1/meta` from R2; deploy workflow green on workers.dev.
- **M1 — Europe-wide stations:** ingest Sensor.Community + OpenAQ in Actions; EAQI scoring; `stations.geojson.gz` to R2; map shows ~13–14k EAQI-colored points with source-class filter and staleness handling. *This alone completes brief goals #1 and #2.*
...
- Payload strategy if station count outgrows ~1–2 MB gz (openSenseMap, network growth): switch `/api/v1/stations` to quantized tile objects — R2 layout supports it without breaking the contract.
- **LAU/municipality-level area mode** (custom-domain milestone): 98k polygons → PMTiles on R2, measured 48.5 MB at z4–z10 — needs the custom domain + edge cache first (see §5.5).
- PMTiles Europe extract size check (basemap fallback readiness).
