...
> **Status (2026-07-21):** this is the planning and decision record,
> maintained through the production launch on 2026-07-21
> (https://aerismap.jiri-hradec-jr.workers.dev). It is kept as history — where
> the shipped system diverged from the plan, a dated bracketed addendum marks
> the divergence rather than rewriting the original text. For current-state
> reference, see [docs/operations.md](docs/operations.md),
...
*[2026-07-21, as shipped: both workflows pin `node-version: 24` (the "Node 22" here and in the §2 diagram is the plan-time floor — the code requires only Node ≥ 22), and the ingest entrypoint lives at `ingest/src/run.ts`, not `ingest/run.ts`.]*
...
- **KV budget (Workers Free plan):** writes 5 keys/hour = ~120/day of the **1,000 writes/day** cap *[2026-07-21: the 5-key figure describes the M2-complete artifact set; the shipped M1 ingest writes 3 keys/run (stations, areas, meta) plus a roughly daily `internal/openaq-registry.json` refresh ≈ 73 writes/day — the ~120 remains the correct planning ceiling]*; reads draw on **100k KV reads/day** (aligned with the Worker's own 100k req/day, one KV read per API hit); storage: `latest/` artifacts total ~0.42 MB gz of the **1 GB** namespace cap, largest value ~398 KB of the **25 MiB/value** cap. Over-cap behavior is failed requests for the rest of the UTC day — the Worker keeps serving the last stored (stale) artifacts, and there is never a bill (no card on the account, no billing surface). One KV nuance accepted: writes are eventually consistent across edge locations (~60 s), irrelevant at hourly cadence.

---

## 5. Ingestion pipeline (GitHub Actions, hourly)

One TypeScript entrypoint (`ingest/run.ts`), four stages, ~2–3 min wall-clock. Every upstream fetch sends `User-Agent: AerisMap/2.0 (+repo URL; contact email)` — mandatory for Sensor.Community, good manners everywhere. *[2026-07-21: the shipped string (`ingest/src/http.ts`) is `AerisMap/2.0 (+https://github.com/aerismap; hosted ingest)` — it carries neither the actual repo URL (github.com/hradecji/aerismap) nor a contact email, so this promise is not yet met as stated; fixing the string is an open follow-up.]*
...
3. Join on `location.id`; project to compact records; drop `indoor==1`, non-European country codes, and `exact_location==0` handling (coarsened coords → flag, don't drop). *[2026-07-21: as shipped, the Europe filter is coordinate-based (`EUROPE_BBOX`), not country-code-based — country codes are recorded but never filtered on, so a station labelled `DE` outside the bbox is dropped and one with a junk country inside it is kept.]*
...
- **UX: auto areas→dots crossfade.** No layer toggle — zooming in fades the choropleth out and the station circles in, so the handoff is continuous and the per-station detail is never more than a zoom away. *[2026-07-21: revised in the shipped UI — the auto crossfade stayed the default, but the Layers panel gained an explicit Regions mode radio (Auto / Points only, `AreaMode`); see [docs/frontend.md](docs/frontend.md).]*

Architecture: the boundary GeoJSONs are **immutable static assets** (`/boundaries/nuts2.geojson`, `/boundaries/nuts3.geojson`, ~450 KB gz total), prepared once by `ingest/scripts/prepare-boundaries.ts` and vendored into the repo — they never touch KV or the hourly job. The hourly ingest publishes only the *values*: `latest/areas.json.gz` (~9 KB gz — `AreaSnapshot`, one flat map keyed by `NUTS_ID`), served at `GET /api/v1/areas`. The frontend joins values onto geometry client-side via MapLibre **feature-state** (`promoteId: 'NUTS_ID'`), so an hourly refresh moves ~9 KB, not ~450 KB. *[2026-07-21: after graduated gating (§5.6) lit up far more regions, the measured artifact is ~21 KB gz — the payload argument is unchanged.]*
...
- **Hotspot promotion** (`HOTSPOT_MIN_BAND = 4`): a station gets `properties.hotspot = true` when its worst **unflagged** pollutant band is ≥ 4 **and** either `kind === 'reference'` or ≥ 1 neighbor within 50 km has a fresh same-pollutant reading within 1 EAQI band. Corroborated epicenters stay prominently visible even at choropleth zooms — QC removes liars without hiding real events. *[2026-07-21, contrast rule added at launch: ring **display** is now conditional, not unconditional. Ingest stamps `regionBands {n2, n3}` (the station's regions' EAQI) onto hotspot stations, and the client draws the ring only when the station's band **exceeds** the region band at the displayed NUTS level (stations in uncolored/unassigned regions always show theirs), and only in the Air Quality view. The promotion rule above is unchanged; see [docs/frontend.md](docs/frontend.md) for the rendering rule.]*
...
*[2026-07-21: **M0 and M1 shipped to production today** — including area mode (§5.5) and spatial QC (§5.6), live at https://aerismap.jiri-hradec-jr.workers.dev, with the two M2 API routes 404ing as planned. M2, M3 and the stretch item remain open.]*
...
- EAQI band cross-check against EEA primary docs (pre-launch task, M1). *[2026-07-21: the launch happened with this still open — the shipped bands are the v1-verified `eaqi-2025` tables (§5.4, `packages/shared/src/eaqi.ts`), and the cross-check against EEA primary documentation is now an overdue post-launch task, not a pre-launch one. The same applies to the reminder comment in `eaqi.ts`.]*
